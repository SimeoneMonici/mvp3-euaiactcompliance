...
st.set_page_config(page_title="AI Compliance v19.18", layout="wide")
st.title("AI Compliance Analyzer - **v19.18**")
...
if st.session_state.step == 0:
    ...
    with st.expander("Governance", expanded=True):
        for q in [q for q in questions if q["step"] == 0]:
...
# --- STEP 1: INVENTORY BASE ---
    ...
    for i in range(num):
        with st.expander(f"Sistema {i+1}", expanded=i == st.session_state.current_system):
            ...
            # --- PULISCI ESCLUSIONI ---
            for excl_id in ["q2_92", "q2_93", "q2_94", "q2_95"]:
                sys.pop(excl_id, None)
            # --- DOMANDE BASE ---
            with st.form(f"base_{i}"):
                for q in [q for q in per_system_qs if q["step"] == 1]:
                ...
                if st.form_submit_button("Salva Sistema"):
                    ...
                    st.rerun()
            if not all(sys.get(q["id"]) for q in per_system_qs if q["step"] == 1):
                all_saved = False
...
# --- STEP 2: DOMANDE DETTAGLIATE + ESCLUSIONI ---
elif st.session_state.step == 2:
    st.header("**Step 2: Domande Dettagliate**")
    all_saved = True
    skip_step3 = False
    for i in range(st.session_state.num_systems):
        with st.expander(f"Sistema {i+1} – {st.session_state.system_answers[i].get('q2_1', '')}", expanded=i == st.session_state.current_system):
            sys = st.session_state.system_answers[i]
            role = sys.get("q2_4", "")
            # --- ESCLUSIONI IN STEP 2 ---
            with st.expander("**Esclusioni**", expanded=True):
                for q in [q for q in per_system_qs if q["id"] in ["q2_92", "q2_93", "q2_94", "q2_95"]]:
                    sys[q["id"]] = render_question(q, f"{q['id']}_{i}", sys.get(q["id"], ""))
                    if q.get("ref"):
                        st.caption(f"**Riferimento:** {q['ref']}")
            # --- CONTROLLO ESCLUSIONE ---
            excluded = any(sys.get(q["id"]) == "Sì" for q in per_system_qs if q["id"] in ["q2_92", "q2_93", "q2_94", "q2_95"])
            if excluded:
                st.success("✅ **ESCLUSO DALL'AI ACT** – 100% conformità")
                skip_step3 = True
                continue
            # --- DOMANDE NORMALI ---
            with st.form(f"detailed_{i}"):
                expanders = {}
                for q in [q for q in per_system_qs if q["step"] == 2 and q["id"] not in ["q2_92", "q2_93", "q2_94", "q2_95"]]:
                    ...
                    if show:
                        expander = q.get("expander", "Obblighi Generali")
                        if expander not in expanders:
                            expanders[expander] = []
                        expanders[expander].append(q)
                # --- EXTRA SETTORI ---
                settore = sys.get("q2_5", "")
                if settore in extra_qs:
                    expander_name = f"Extra {settore}"
                    expanders[expander_name] = []
                    for eq in extra_qs[settore]:
                        ...
                        if show:
                            expanders[expander_name].append(eq)
                # --- RENDER ---
                for name, qs in expanders.items():
                ...
                if st.form_submit_button("Salva Sistema"):
                    st.session_state.current_system = i
                    st.rerun()
            # --- VALIDAZIONE (ESCLUDE ESCLUSIONI) ---
            required_qs = [q for q in per_system_qs if q["step"] == 2 and q["id"] not in ["q2_92", "q2_93", "q2_94", "q2_95"]]
            if not all(sys.get(q["id"], "") != "" for q in required_qs if (q.get("condition") is None or all(sys.get(k, "") == v for k, v in q.get("condition", {}).items()))):
                all_saved = False
                st.warning(f"Sistema {i+1}: Compila tutte le domande obbligatorie")
    st.session_state.skip_step3 = skip_step3
    if all_saved:
        if st.button("Avanti"):
            if skip_step3:
        ...
        st.error("Compila tutte le domande prima di procedere")

# --- STEP 3: SPECIFICHE AVANZATE ---
...
# --- STEP 5: RISULTATI ---
    ...
    st.header("**Step 5: Risultati v19.18**")
    if st.session_state.results is None:
        st.error("Nessun risultato calcolato. Torna indietro e premi 'Calcola Risultati'.")
    else:
        ...
        for gid, g in gaps.items():
            if gid.startswith('q1_'):
                gap_list.append({"Sistema": "Generale", "Gap": f"{gid}: {g['question']}", "Risposta": str(g["response"]), "Risk": g["risk_flag"]})
        for sys_gaps in system_gaps_list:
            for gap in sys_gaps["Gaps"]:
                gap["Sistema"] = sys_gaps["system_name"]
                gap["Risposta"] = str(gap["Risposta"])
                gap_list.append(gap)
        if gap_list:
            df = pd.DataFrame(gap_list)
            st.dataframe(df, width="stretch")  # <-- FIX use_container_width
        else:
        ...
        for gap in gap_list:
            if isinstance(gap, dict) and "Gap" in gap:
                ...
                if r:
                    ...
                    elif "risposta < 3" in cond and str(gap["Risposta"]).isdigit() and int(gap["Risposta"]) < 3:
                        roadmap.append({
                            "Sistema": gap["Sistema"],
                            "Gap": gap["Gap"],
                            "Azione": r["recommendation"],
        ...
        with col1:
            st.download_button("Excel Completo", excel, "AI_Compliance_v19.18.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        with col2:
            st.download_button("Roadmap CSV", csv_data, "Roadmap_v19.18.csv", "text/csv")
        # --- DEBUG STRUTTURATO ---
...
st.caption("AI Compliance Analyzer v19.18 – 2025")